...
class TeXOutlineProvider implements vscode.TreeDataProvider<vscode.TreeItem> {
  ...
  private lastTeXDocument: vscode.TextDocument | undefined =
    isTeXDocument(vscode.window.activeTextEditor?.document)
  ...
  private getActiveTeXDocument(): vscode.TextDocument | undefined {
      ...
      if (isTeXDocument(doc)) {
        this.lastTeXDocument = doc;
        return doc;
      }

      if (this.lastTeXDocument && !this.lastTeXDocument.isClosed) {
        return this.lastTeXDocument;
      }

      const fallback = vscode.workspace.textDocuments.find(
        (d) => isTeXDocument(d) && !d.isClosed
      );
      if (fallback) {
        this.lastTeXDocument = fallback;
        return fallback;
      }
